function [ im_out ] =Coupled_frame_denoising(img_x,img_xhat, dict1, dict2,lamuda,epsilon)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[row,col] = size(img_x);
scalar = diag(dict1'*dict1);
filterSize = sqrt(size(dict1,1));
im_out = zeros(row,col);
for i = 1:size(dict1, 2);
    %% update the coeffiencient of the orignal image 
    kernel1 = reshape(dict1(:,i),filterSize,filterSize);
    temp1   = filter2(kernel1, img_x, 'valid');
  %% update the coeffiencient of the orignal image    
    kernel2= reshape(dict2(:,i),filterSize,filterSize);
    temp2   = filter2(kernel2, img_xhat, 'valid');
    temp=(temp1+lamuda*temp2)./(1+lamuda);
  %  if i==1
